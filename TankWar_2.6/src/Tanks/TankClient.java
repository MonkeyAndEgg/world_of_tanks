...
public class TankClient extends Frame{
	...
	public void paint(Graphics g) {
		...
		// if game is over, then go to final page
		if(myTank.getNumOfReborn() == 0 && !myTank.isLive()) {
			g.setFont(new Font("Courier New", 1, 36));
			this.gameOver = true;
			g.setColor(Color.WHITE);
			g.drawString("Game Over",300, 280);
			g.setFont(new Font("Courier New", 1, 20));
			g.drawString("Press ENTER to restart :D", 255, 340);
			g.setFont(new Font("Courier New", 1, 15));
			g.drawString("Written By MonkeyAndEgg", 580, 592);
			g.setFont(f);
		...
		g.drawString("Lives left:" + myTank.getNumOfReborn(), 10, 130);
