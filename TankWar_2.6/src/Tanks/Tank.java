...
public class Tank {
	...
	public void keyPressed (KeyEvent e) { 
		...
		switch (key) {
			...
			// revive
			case KeyEvent.VK_R:
				if(!this.live && this.numOfReborn > 0) {
					this.live = true;
					this.hp = 100;
					this.numOfReborn--;
				}
			...
			case KeyEvent.VK_ENTER:
				if(this.numOfReborn == 0 && tc.isGameOver()) {
					tc.setGameOver(false);
	...
	public boolean consume (FirstAid aid) {
		if(this.live && (hp != 100) && this.getRect().intersects(aid.getRect())) {
			setHp(100);
			tc.aid.setLive(false);  // the aid is eaten
			return true;
		}
		return false;
	}
	...
	public int getNumOfReborn() {
		return numOfReborn;
	}

	public void setNumOfReborn(int numOfReborn) {
		this.numOfReborn = numOfReborn;
	}
